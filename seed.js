...
async function seed() {
  try {
    ...
    console.log(`   ${questions.length} questions across 4 categories`);
    console.log(`   ${quizzes.length} preset quizzes created`);
  } catch (err) {
...
// 4 preset quizzes — one per category
const quizzes = [
  ...
  try {
    ...
    for (const q of questions) {
      const [result] = await conn.execute(
        `INSERT INTO questions (text, type, category, difficulty, options, correct_index, language)
         VALUES (?, ?, ?, ?, ?, ?, ?)`,
        [q.text, q.type, q.category, q.difficulty, q.options, q.correct_index, q.language]
      );
      questionIds.push(result.insertId);
    }
    console.log(`✓ Inserted ${questionIds.length} questions`);

    // Insert preset quizzes (created_by = NULL for admin-seeded content)
    for (const quiz of quizzes) {
      const [quizResult] = await conn.execute(
        `INSERT INTO quizzes (title, category, created_by, is_preset, language)
         VALUES (?, ?, NULL, TRUE, 'fr')`,
